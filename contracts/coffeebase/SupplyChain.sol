...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole{
  ...
  mapping (uint => string[]) itemsHistory; //   mapping (uint => string[]) itemsHistory;
...
string memory _originFarmLatitude, string memory _originFarmLongitude, string memory  _productNotes) public onlyFarmer() {
    ...
    //string memory productId = string.concat(Strings.toString(_upc),"+",Strings.toString(sku));
  ...
  function processItem(uint _upc) public harvested(_upc) onlyFarmer(){ 
    // Call modifier to check if upc has passed previous supply chain stage and verify if the caller of this function is a farmer
  ...
  // Call modifier so that only distributor can call this function
    address payable farmerWallet = payable(items[_upc].originFarmerID);
    items[_upc].ownerID = msg.sender;// Update the appropriate fields - ownerID, distributorID, itemState
